...
export const getTrendingItems = async (media_type, timeFrame) => {
  const response = await fetch(
    `https://api.themoviedb.org/3/trending/${media_type}/${timeFrame}?api_key=${process.env.REACT_APP_TMDB_KEY}`
  );
